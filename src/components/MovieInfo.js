...
import {addMovieInfo, addCastInfo, addMovieTrailers} from '../utils/moviesSlice';
...
const MovieInfo = () => {
    ...
    useEffect(() => {
        ...
        const getMovieDetails = async() => {
            try{    
                const data = await fetch(`https://api.themoviedb.org/3/movie/${id}?language=en-US`, API_OPTION);
        ...
        const getCastDetails = async() => {
            try{
                const data = await fetch(`https://api.themoviedb.org/3/movie/${id}/credits`, API_OPTION);
